...
class Athena:
    ...
    def GenerateCard(self, item: dict):
        ...
        try:
            ...
            category = item["items"][0]["type"]["displayValue"]
        ...
        rarit = item["items"][0]["rarity"]["displayValue"]

        font = ImageUtil.Font(self, 30)
        textWidth, _ = font.getsize(f"{rarit.capitalize()} {category.capitalize()}")
        canvas.text(
            ImageUtil.CenterX(self, textWidth, card.width, 385),
            f"{rarit.capitalize()} {category.capitalize()}",
            blendColor,
            font=font,
        )
...
if __name__ == "__main__":
    try:
        ...
        log.info("Chargement du bot Discord...")
